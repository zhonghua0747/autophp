...
function dh_replace_link($sql,$row,$DH_output_content)
{
	...
	while($rowlinks = mysql_fetch_array($reslinks))
	{
		...
		$linkseach1 = '<li><div class="listlink">%num% <span class="lqc'.$quality.'">['.$linkquality[$quality].']</span> <a href = "'. $rowlinks['link'] . '" target = "_blank" title="点击访问链接:'.$title.'('.$rowlinks['author'].')" rel="nofollow">'.$title. '</a></div><div class="lqc3 rt0v5"> '.$rowlinks['author'].' <span class="c'.$type.'">'.$linktype[$type].'</span> '.$updatetime.'</div></li>';
		$linkseach2 = '<li><div class="listlink">%num% <a href = "'. $rowlinks['link'] . '" target = "_blank" title="点击访问链接:'.$title.'('.$rowlinks['author'].')" rel="nofollow">'.$title. '</a></div><div class="lqc3 rt0v5"> '.$rowlinks['author'].' '.$updatetime.'</div></li>';			
		switch ($way)
		{
			case 1://资讯
				$num1++;
